...
io.on('connection', (socket) => {
  ...
  socket.on('join', (params, callback) => {
    ...
    socket.emit('newMessage', generateMessage('Admin', 'Welcome to the Node Messenger'));
  ...
  socket.on('createMessage', (message, callback) => {
    ...
    if(user && isRealString(message.text)){
      socket.emit('newSelfMessage', generateMessage('Me', message.text));
  ...
  socket.on('createLocationMessage', (coords) => {
    ...
    if(user){
      socket.emit('newSelfLocationMessage', generateLocationMessage('Me', coords.latitude, coords.longitude));
